...
- Generate or update VDD's `vdd_settings.xml` for the requested display count and square MFD resolution
- Attempt to reload the VDD adapter so the new display count takes effect immediately
- Read your active Saved Games `Config\options.lua` monitor preset and resolution
- Generate a tailored `AccMod_MFD_Auto.lua` MonitorSetup template in your DCS profile
...
If you ran `setup-mfd-system.ps1`, the script already writes the VDD XML so the driver exposes the requested number of square displays. You still need to verify them in Windows Display Settings and arrange them where you want them.
...
If you ran `setup-mfd-system.ps1`, it also generates:
```
%USERPROFILE%\Saved Games\<Your DCS Profile>\Config\MonitorSetup\AccMod_MFD_Auto.lua
```

That generated file uses your current `options.lua` desktop width and height so the main viewport matches your existing setup, and it places the MFD exports immediately to the right of the current desktop span as a starting point.
