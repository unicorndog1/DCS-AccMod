...
local MULTIPLAYER_OVERLAY_DEBUG_LOGGING = false

local function formatMultiplayerDebugValue(value, depth)
    ...
    local parts = {}
    local limit = math.min(#keys, 6)
    for index = 1, limit do
        local key = keys[index]
        parts[#parts + 1] = tostring(key) .. "=" .. formatMultiplayerDebugValue(value[key], depth + 1)
    end
    if #keys > limit then
        parts[#parts + 1] = "..."
    end
...
local function collectMultiplayerOverlayDiagnostics()
    local diagnostics = {
        netAvailable = (type(net) == "table"),
        hasGetMyPlayerId = net and type(net.get_my_player_id) == "function" or false,
        hasGetServerId = net and type(net.get_server_id) == "function" or false,
        hasGetPlayerList = net and type(net.get_player_list) == "function" or false,
        hasGetPlayerInfo = net and type(net.get_player_info) == "function" or false,
    }

    if diagnostics.hasGetMyPlayerId then
        diagnostics.okPlayerId, diagnostics.playerId = pcall(net.get_my_player_id)
        if not diagnostics.okPlayerId then
            diagnostics.playerIdError = diagnostics.playerId
            diagnostics.playerId = nil
        end
    end

    if diagnostics.hasGetServerId then
        diagnostics.okServerId, diagnostics.serverId = pcall(net.get_server_id)
        if not diagnostics.okServerId then
            diagnostics.serverIdError = diagnostics.serverId
            diagnostics.serverId = nil
        end
    end

    if diagnostics.hasGetPlayerList then
        diagnostics.okPlayerList, diagnostics.playerList = pcall(net.get_player_list)
        if diagnostics.okPlayerList and type(diagnostics.playerList) == "table" then
            diagnostics.playerCount = #diagnostics.playerList
        elseif not diagnostics.okPlayerList then
            diagnostics.playerListError = diagnostics.playerList
            diagnostics.playerList = nil
        end
    end

    if diagnostics.hasGetPlayerInfo and type(diagnostics.playerId) == "number" then
        diagnostics.okMyPlayerInfo, diagnostics.myPlayerInfo = pcall(net.get_player_info, diagnostics.playerId)
        if not diagnostics.okMyPlayerInfo then
            diagnostics.myPlayerInfoError = diagnostics.myPlayerInfo
            diagnostics.myPlayerInfo = nil
        end
    end

    diagnostics.blocked = false
    diagnostics.reason = "net ids did not indicate multiplayer client"

    if not diagnostics.netAvailable then
        diagnostics.reason = "net singleton unavailable"
    elseif not diagnostics.hasGetMyPlayerId or not diagnostics.hasGetServerId then
        diagnostics.reason = "required net id functions unavailable"
    elseif diagnostics.okPlayerId == false or diagnostics.okServerId == false then
        diagnostics.reason = "net id calls failed"
    elseif type(diagnostics.playerId) ~= "number" or type(diagnostics.serverId) ~= "number" then
        diagnostics.reason = "net id calls returned non-number values"
    elseif diagnostics.playerId > 0 and diagnostics.serverId > 0 and diagnostics.playerId ~= diagnostics.serverId then
        diagnostics.blocked = true
        diagnostics.reason = "local player id differs from server id"
    end

    return diagnostics
end

local function maybeLogMultiplayerOverlayDiagnostics(managerInstance, reason, diagnostics, force)
    if not managerInstance or MULTIPLAYER_OVERLAY_DEBUG_LOGGING ~= true then
        return
    end

    diagnostics = diagnostics or collectMultiplayerOverlayDiagnostics()
    local playerListSummary = formatMultiplayerDebugValue(diagnostics.playerList)
    local playerInfoSummary = formatMultiplayerDebugValue(diagnostics.myPlayerInfo)
    local signature = table.concat({
        tostring(diagnostics.netAvailable),
        tostring(diagnostics.playerId),
        tostring(diagnostics.serverId),
        tostring(diagnostics.playerCount),
        tostring(diagnostics.blocked),
        tostring(diagnostics.reason),
        tostring(playerListSummary),
        tostring(playerInfoSummary)
    }, "|")

    local now = os.clock()
    local shouldLog = force == true
        or managerInstance.lastMultiplayerOverlayLogSignature ~= signature
        or not managerInstance.lastMultiplayerOverlayLogTime
        or (now - managerInstance.lastMultiplayerOverlayLogTime) >= 5

    if not shouldLog then
        return
    end

    managerInstance.lastMultiplayerOverlayLogTime = now
    managerInstance.lastMultiplayerOverlayLogSignature = signature

    log.write('AccMod', log.INFO,
        string.format(
            "MP overlay diag [%s] blocked=%s reason=%s net=%s getMy=%s getServer=%s getList=%s getInfo=%s playerId=%s serverId=%s playerCount=%s playerList=%s myInfo=%s playerIdErr=%s serverIdErr=%s playerListErr=%s myInfoErr=%s",
            tostring(reason or "unknown"),
            tostring(diagnostics.blocked),
            tostring(diagnostics.reason),
            tostring(diagnostics.netAvailable),
            tostring(diagnostics.hasGetMyPlayerId),
            tostring(diagnostics.hasGetServerId),
            tostring(diagnostics.hasGetPlayerList),
            tostring(diagnostics.hasGetPlayerInfo),
            tostring(diagnostics.playerId),
            tostring(diagnostics.serverId),
            tostring(diagnostics.playerCount),
            tostring(playerListSummary),
            tostring(playerInfoSummary),
            tostring(diagnostics.playerIdError),
            tostring(diagnostics.serverIdError),
            tostring(diagnostics.playerListError),
            tostring(diagnostics.myPlayerInfoError)
        )
    )
end

local function syncMultiplayerOverlayRestriction(managerInstance, reason, forceLog)
    if not managerInstance then
        return false
    end

    if managerInstance.multiplayerOverlayBypass == true then
        if managerInstance.multiplayerVrOverlayBlocked == true then
            managerInstance.multiplayerVrOverlayBlocked = false
            log.write('AccMod', log.INFO,
                "Multiplayer overlay restriction bypass enabled")
        end
        return false
    end

    local diagnostics = collectMultiplayerOverlayDiagnostics()
    maybeLogMultiplayerOverlayDiagnostics(managerInstance, reason, diagnostics, forceLog)

    if diagnostics.blocked then
        local modeChanged = (managerInstance.vrModeEnabled or 0) ~= 0
        managerInstance.vrModeEnabled = 0

        if managerInstance.vrButtonWidget then
            managerInstance.vrButtonWidget:setText("VR Mode: DISABLED (MP)")
        end
        if managerInstance.vrResetButtonWidget then
            managerInstance.vrResetButtonWidget:setVisible(false)
        end
        ...
        if modeChanged then
            ...
            log.write('AccMod', log.WARNING,
                "VR overlay disabled for multiplayer client session: " .. tostring(diagnostics.reason))
        elseif managerInstance.multiplayerVrOverlayBlocked ~= true then
            log.write('AccMod', log.INFO,
                "VR overlay locked while connected to multiplayer server: " .. tostring(diagnostics.reason))
        end

        managerInstance.multiplayerVrOverlayBlocked = true
        return true
    end
    ...
    if managerInstance.multiplayerVrOverlayBlocked == true then
        managerInstance.multiplayerVrOverlayBlocked = false
        log.write('AccMod', log.INFO,
            "VR overlay multiplayer restriction cleared: " .. tostring(diagnostics.reason))
    end
...
local function getManagerRenderModeButtonText(manager, panel)
    if manager and manager.multiplayerVrOverlayBlocked == true then
        return "Mode: Blocked (MP)"
    end
...
local function ensureUnitHighlightPanelForMode()
    ...
    if manager.multiplayerVrOverlayBlocked == true then
        if manager.unitHighlightPanel and manager.unitHighlightPanel.window then
            ...
            log.write('AccMod', log.INFO, "Unit Highlighter panel disabled by multiplayer restriction")
        end
        return
    end
...
local function performVrModeToggle(managerInstance, vrButton, vrResetButton)
    ...
    if syncMultiplayerOverlayRestriction(managerInstance, "toggle", true) then
        ensureUnitHighlightPanelForMode()
        syncManagerRenderModeUi()
        return
    end
...
local function syncVrButtonState(managerInstance)
    ...
    if syncMultiplayerOverlayRestriction(managerInstance, "sync-button", false) then
        ensureUnitHighlightPanelForMode()
        syncManagerRenderModeUi()
        return
    end
...
function UnitHighlightPanel:createWindow()
    ...
    infoSkin.skinData.states.released[1].text.fontSize = 11
    ...
    debugSkin.skinData.states.released[1].text.fontSize = 9
...
local function safeExportCall(functionName)
    if type(base.Export) ~= "table" then
        return false, nil, "Export API unavailable"
    end

    local exportFn = base.Export[functionName]
    if type(exportFn) ~= "function" then
        return false, nil, functionName .. " unavailable"
    end

    local ok, result = pcall(exportFn)
    if not ok then
        return false, nil, functionName .. " failed: " .. tostring(result)
    end

    return true, result, nil
end

local function describeWorldDataFailure(worldCallError, worldObjects)
    local okSelf, selfData, selfErr = safeExportCall("LoGetSelfData")
    local okPlayerPlaneId, playerPlaneId, playerPlaneErr = safeExportCall("LoGetPlayerPlaneId")

    if okPlayerPlaneId and type(playerPlaneId) == "number" then
        cachedPlayerPlaneId = playerPlaneId
    end

    local isMultiplayerClient = false
    if type(net) == "table"
        and type(net.get_my_player_id) == "function"
        and type(net.get_server_id) == "function" then
        local okPlayerId, playerId = pcall(net.get_my_player_id)
        local okServerId, serverId = pcall(net.get_server_id)
        isMultiplayerClient = okPlayerId
            and okServerId
            and type(playerId) == "number"
            and type(serverId) == "number"
            and playerId > 0
            and serverId > 0
            and playerId ~= serverId
    end

    if not okSelf or type(selfData) ~= "table" then
        return "No world data: " .. tostring(selfErr or "LoGetSelfData unavailable")
    end

    if not selfData.Name or not selfData.Position then
        return "No world data: local player not fully spawned yet"
    end

    if not okPlayerPlaneId then
        return "No world data: " .. tostring(playerPlaneErr or "LoGetPlayerPlaneId unavailable")
    end

    if type(playerPlaneId) ~= "number" or playerPlaneId <= 0 then
        return "No world data: local player slot not fully initialized"
    end

    if type(worldObjects) ~= "nil" and type(worldObjects) ~= "table" then
        return "No world data: LoGetWorldObjects returned " .. type(worldObjects)
    end

    if worldCallError then
        return "No world data: " .. tostring(worldCallError)
    end

    if isMultiplayerClient then
        return "No world data: multiplayer world object export unavailable in this session"
    end

    return "No world data: LoGetWorldObjects returned nil after spawn"
end
...
function UnitHighlightPanel:detectUnits()
    ...
    local okWorldObjects, worldObjects, worldObjectsError = safeExportCall("LoGetWorldObjects")
    if not okWorldObjects or type(worldObjects) ~= "table" then
        return nil, describeWorldDataFailure(worldObjectsError, worldObjects)
    end

    local okSelfData, selfData, selfDataError = safeExportCall("LoGetSelfData")
    if not okSelfData or type(selfData) ~= "table" then
        local selfDataReason = selfDataError
        if not selfDataReason then
            selfDataReason = "LoGetSelfData returned " .. type(selfData)
        end
        return nil, "No camera data: " .. tostring(selfDataReason)
    end
    ...
    if not camera then
        local okCamera, cameraResult, cameraError = safeExportCall("LoGetCameraPosition")
        if not okCamera or type(cameraResult) ~= "table" then
            local cameraReason = cameraError
            if not cameraReason then
                cameraReason = "LoGetCameraPosition returned " .. type(cameraResult)
            end
            return nil, "No camera data: " .. tostring(cameraReason)
        end
        camera = cameraResult
    end
    if not camera or not camera.p or not camera.x then
...
function UnitHighlightPanel:update()
    ...
    if not detectedUnits then
        ...
        self.detectedUnits = {}
        self:hideAllUnitMarkers()
        if errorMsg and self.lastWorldDataError ~= errorMsg then
            log.write('AccMod', log.WARNING, "UnitHighlightPanel unavailable: " .. tostring(errorMsg))
        end
        self.lastWorldDataError = errorMsg
        return
    end

    self.lastWorldDataError = nil
...
function DebugInfoPanel:createWindow()
    ...
    for i = 1, self.maxLines do
        ...
        if not textSkin.skinData.states.released[1].text then
            ...
            textSkin.skinData.states.released[1].text.fontSize = 7
...
AccModOverlayManager = {
    ...
    multiplayerOverlayBypass = true, -- When true, do not block overlays in multiplayer
    ...
    lastMultiplayerOverlayLogTime = 0,
    lastMultiplayerOverlayLogSignature = nil,
...
function AccModOverlayManager:loadConfiguration()
    ...
    if tbl and tbl.config then
        ...
        if self.managerConfig.multiplayerOverlayBypass ~= nil then
            self.multiplayerOverlayBypass = self.multiplayerOverlayBypass == true
                or self.managerConfig.multiplayerOverlayBypass == true
        end
        ...
        self.managerConfig = {
            ...
            panelsEnabled = true,
            multiplayerOverlayBypass = self.multiplayerOverlayBypass == true,
            panels = {},
...
function AccModOverlayManager:saveConfiguration()
    if self.managerConfig then
        ...
        self.managerConfig.multiplayerOverlayBypass = self.multiplayerOverlayBypass == true
...
function AccModOverlayManager:createManagerWindow()
    ...
    tabAccessibility:setBounds(0, 0, 95, 24)
    tabTools:setBounds(95, 0, 95, 24)
    tabLabels:setBounds(190, 0, 95, 24)
    tabKeybinds:setBounds(285, 0, 95, 24)
    tabUnitPlacer:setVisible(false)
    ...
    unitPlacerPanel:setVisible(false)
    ...
    btnShowAll:setBounds(160, 155, 140, 28)
...
function AccModOverlayManager.onSimulationFrame()
    syncMultiplayerOverlayRestriction(AccModOverlayManager, "frame", false)
    ensureUnitHighlightPanelForMode()
...
-- Combined callbacks object to avoid overwriting handlers
local CombinedCallbacks = {}

function CombinedCallbacks.onSimulationFrame()
    -- Call AccModOverlayManager's frame handler
    if AccModOverlayManager and AccModOverlayManager.onSimulationFrame then
        AccModOverlayManager.onSimulationFrame()
    end
    
    -- Call JankyJoy's frame handler
    if JankyJoy and JankyJoy.onSimulationFrame then
        JankyJoy:onSimulationFrame()
    end
end

function CombinedCallbacks.onSimulationStop()
    -- Call AccModOverlayManager's stop handler if it exists
    if AccModOverlayManager and AccModOverlayManager.onSimulationStop then
        AccModOverlayManager:onSimulationStop()
    end
    
    -- Call JankyJoy's stop handler
    if JankyJoy and JankyJoy.onSimulationStop then
        JankyJoy:onSimulationStop()
    end
end

DCS.setUserCallbacks(CombinedCallbacks)
-- 1.0 76
-- .72 55
-- .26 35
-- -.18 22
